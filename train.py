...
def main(args):
    ...
    data = hag.utils.load_pkl(args.data)
    log.info("Loaded data.")

    trainset = hag.Dataset(data["train"], args.batch_size)
    devset = hag.Dataset(data["dev"], args.batch_size)
    testset = hag.Dataset(data["test"], args.batch_size)

    log.debug("Building model...")
    model_file = "./save/model_"+args.modelinfo+".pt"
    model = hag.HAG(args).to(args.device)
    opt = hag.Optim(args.learning_rate, args.max_grad_value, args.weight_decay)
    opt.set_parameters(model.parameters(), args.optimizer)

    coach = hag.Coach(trainset, devset, testset, model, opt, args)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--hidden_size", type=int, default=268,
    parser.add_argument("--hidden_size", type=int, default=100,
                        help="Hidden size of two layer GNN.")
    ...
    parser.add_argument("--seed", type=int, default=66,
                        help="Random seed.")
    parser.add_argument("--modelinfo", type=str, default='',
                        help="model information of HP.")
